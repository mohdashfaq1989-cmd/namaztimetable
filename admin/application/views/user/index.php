                        ...
                        <div class="jarviswidget jarviswidget-color-blueDark" id="wid-id-1"
                            ...
                            <div>
                                ...
                                <div class="widget-body no-padding">
                                    ...
                                    <a  href="<?php echo base_url('user/add'); ?>" class="btn btn-primary">Add <?php echo $page_title; ?></a>
                                </div>
                                        ...
                                        <tbody> 
							                 <?php if(!empty($gallery)){ $i=0;  
							                    foreach($gallery as $row){ $i++; 
							                        ...
							                        $statusLink = ($row['status'] == 1)?site_url('user/block/'.$row['id']):site_url('user/unblock/'.$row['id']);  
							                        $statusTooltip = ($row['status'] == 1)?'Click to Inactive':'Click to Active';  
							                ?>
							                <tr>
							                    <td><?php echo $i; ?></td>
							                    <td><?php echo $image; ?></td>
                                                <td><?php  if($row['role_id']==1): echo "Imam"; endif;?> 
                                                    <?php  if($row['role_id']==2): echo "Mutwalli"; endif;?>
                                                    <?php  if($row['role_id']==3): echo "Student"; endif;?>                
                                                </td>
							                    ...
							                    <td>
							                        <a href="<?= base_url('user/edit/'.$row['id']) ?>" class="btn btn-warning"><i class="fa fa-pencil"></i></a>
            ...
            $.ajax({    
                     ...
                     {  
                        ...
                        if(response['status']==1){ 
                            $('#modal_msg').html('User deleted Successfully').css('color','green');
                            setTimeout(function(){
                                $('#modal_msg').html('');
                                location.reload();
                            },1000)
