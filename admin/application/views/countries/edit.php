...
<script src="<?php echo base_url()  ?>assets/ckeditor/sample.js"></script>
    ...
    <input type="hidden" name="country_id" value="<?php echo !empty($dbData['country_id'])?$dbData['country_id']:''; ?>" />

    <div class="form-group row">
        <label for="name" class="col-sm-3 col-form-label">Country Code</label>
        <div class="col-sm-9">
            <input name="countryCode" class="form-control" type="text" placeholder="Country Code" id="countryCode" value="<?php echo !empty($dbData['countryCode'])?$dbData['countryCode']:''; ?>" />
        </div>
    </div>
    <div class="form-group row">
        <label for="name" class="col-sm-3 col-form-label">Country Name</label>
        <div class="col-sm-9">
            <input name="country_name" class="form-control" type="text" placeholder="Country Name" id="country_name" value="<?php echo !empty($dbData['country_name'])?$dbData['country_name']:''; ?>" />
        </div>
    </div>
	...
	$(document).ready(function(){
        ...
        $('#insertUserData').on('submit', function(e){  
            ...
            $.ajax({    
                     url:"<?php echo base_url(); ?>countries/edit_countries/",    
                     method:"POST",  
                     data:formData,  
                     beforeSend: function() {
