                        ...
                        <div class="jarviswidget jarviswidget-color-blueDark" id="wid-id-1"
                            ...
                            <div>
                                ...
                                <div class="widget-body no-padding">
                                    ...
                                    <a  href="<?php echo base_url('ads/add'); ?>" class="btn btn-primary">Add <?php echo $page_title; ?></a>
                                </div>
                                    <table id="datatable_fixed_column" class="table table-striped table-bordered" 
                                        ...
                                        <thead>
                                            ...
                                            <tr>
                                                ...
                                                <th data-hide="phone,tablet">Ad Image</th>
                                                <th data-hide="phone,tablet">Masjid Name</th>
                                                <th data-hide="phone,tablet">Ad Name</th>
                                        ...
                                        <tbody> 
							                 <?php if(!empty($gallery)){ $i=0;  
							                    foreach($gallery as $row){ $i++; 
							                        ...
							                        $statusLink = ($row['status'] == 1)?site_url('ads/block/'.$row['id']):site_url('ads/unblock/'.$row['id']);  
							                        $statusTooltip = ($row['status'] == 1)?'Click to Inactive':'Click to Active';  
							                ?>
							                <tr>
							                    <td><?php echo $i; ?></td>
							                    <td><?php echo $image; ?></td>
							                    <td><?php echo $this->db->get_where('masjid', array('id' => $row['masjid_id']))->row('name'); ?></td>
							                    ...
							                    <td>
							                        <a href="<?= base_url('ads/edit/'.$row['id']) ?>" class="btn btn-warning"><i class="fa fa-pencil"></i></a>
							                        <a class="btn btn-danger dataDelete" data-type="user" data-id="<?= $row['id'] ?>" data-name="<?= $row['title']?>" href="javascript:;"><i class="fa fa-trash-o"></i></a>
							                    </td>
							                </tr>
							                <?php } }else{ ?>
...
<div class="modal fade" id="DeleteModal" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
 ...
 $(document).ready(function(){ 
        $(document).delegate('.deleteUser', 'click', function () {
            ...
            console.log('modal')
            $.ajax({    
                     url:"<?php echo base_url(); ?>ads/deleteads",       
                     method:"POST",  
                     data:{
                         ads_id:ads_id,
                     },    
                     ...
                     {  
                        ...
                        if(response['status']==1){ 
                            $('#modal_msg').html('ads deleted Successfully').css('color','green');
                            setTimeout(function(){
                                $('#modal_msg').html('');
                                location.reload();
                            },1000)
