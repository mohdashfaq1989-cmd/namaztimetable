...
<script src="<?php echo base_url()  ?>assets/ckeditor/sample.js"></script>
    ...
    <input type="hidden" name="ads_id" value="<?php echo !empty($dbData['id'])?$dbData['id']:''; ?>" />
    <div class="form-group row">
        ...
        <div class="col-sm-9">
            <select id="masjid_id" name="masjid_id" class="form-control" required/>
                ...
                <option value="<?php echo $val->id;?>" <?php if($dbData['masjid_id']==$val->id){echo'selected';} ?>><?php echo $val->name;?></option>
                    <?php  }} ?>
            </select>
        </div>
    </div>
    <div class="form-group row">
        <label for="name" class="col-sm-3 col-form-label">ads Name</label>
        <div class="col-sm-9">
            <input name="title" class="form-control" type="text" placeholder="ads Name" id="title" value="<?php echo !empty($dbData['title'])?$dbData['title']:''; ?>" />
        </div>
    </div>
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-9">
            <textarea name="description" placeholder="Description" class="form-control" id="description" rows="6" ><?php echo !empty($dbData['description'])?$dbData['description']:''; ?></textarea>
        </div>
    </div>
...
<div class="form-group row">
    ...
    <div class="col-sm-9">
        <img id="blah" src="<?php if(!$dbData['image']==""){echo '../uploads/ads/'.$dbData['image'];}else{echo '../assets/img/default.jpg';}?>" class="img-thumbnail" width="125" height="100">
    </div>
	...
	$(document).ready(function(){
        ...
        $('#insertUserData').on('submit', function(e){  
            ...
            $.ajax({    
                     url:"<?php echo base_url(); ?>ads/edit_ads/",    
                     method:"POST",  
                     data:formData,  
                     beforeSend: function() {
