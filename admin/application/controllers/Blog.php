...
class Blog extends CI_Controller { 
    ...
    public function add_blog(){
        ...
        $insertData = $this->db->insert('blog',$formData);
        if($insertData)
        {
    ...
    public function edit_blog(){ 
        ...
        $updateData = $this->db->update('blog',$formData,array('id'=>$blog_id));
        if($updateData)
        {
            $response = array('status'=>1);
        }
    ...
    public function deleteBlog(){
        $blog_id = $this->input->post('blog_id');
        $delete = $this->db->delete('blog', array('id'=>$blog_id));
        if($delete)
        {
            $response = array('status'=>1);
        }
    ...
    public function index(){ 
        ...
        $data['gallery'] = $this->blog_model->getRows(); 
        $data['title'] = 'All Blog'; 
    ...
    public function edit($id){
        ...
        // Get image data 
        $con = array('id' => $id); 
        $imgData = $this->blog_model->getRows($con);  
    ...
    public function block($id){ 
        ...
        if($id){ 
            // Update image status 
            $data = array('status' => 0); 
            $update = $this->blog_model->update($data, $id); 
             
            if($update){ 
                $this->session->set_userdata('success_msg', 'blog has been disable successfully.'); 
            }else{ 
    ...
    public function unblock($id){ 
        ...
        if($id){ 
            ...
            if($update){ 
                $this->session->set_userdata('success_msg', 'blog has been activated successfully.'); 
            }else{ 
                $this->session->set_userdata('error_msg', 'Some problems occurred, please try again.'); 
            } 
        } 
